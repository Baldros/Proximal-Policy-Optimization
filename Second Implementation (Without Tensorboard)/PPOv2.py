...
def get_action_dim(action_space):
    '''
    Espaços discretos e contínuos no Gymnasium possuem métodos
    distintos, deste modo, faz-se necessário, para generalizar
    a implementação, verificar o espaço e atribuir o método
    adequado a cada classe da API gymnasium.
    '''
    if isinstance(action_space, Discrete):
        ...
        raise NotImplementedError(f"Espaço de ações do tipo {type(action_space)} não suportado.")
...
class PPOAgent(nn.Module):
    ...
    def __init__(self, observation_space, action_space):
        ...
        if isinstance(action_space, Discrete):
            ...
            # Para espaços contínuos, a política produz a média das ações. A variância pode ser um parâmetro separado.
            self.actor = nn.Sequential(
                layer_init(nn.Linear(np.prod(obs_shape), 64)),
                nn.Tanh(),
                layer_init(nn.Linear(64, 64)),
            ...
            # Log standard deviation como parâmetro treinável
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if self.action_space_type == Discrete:
            probs = Categorical(logits=logits_or_mean)  # Distribuição para ações discretas
            if action is None:
                action = probs.sample()  # Amostra aleatoriamente uma ação da política atual
            return action, probs.log_prob(action), probs.entropy(), self.critic(x)
        elif self.action_space_type == Box:
            std = self.log_std.exp().expand_as(logits_or_mean)
            dist = Normal(logits_or_mean, std)  # Distribuição para ações contínuas
            if action is None:
                action = dist.sample()  # Amostra aleatoriamente uma ação da política atual
            # Para ações contínuas multivariadas, somamos as log-probabilidades e entropias
            log_prob = dist.log_prob(action).sum(axis=-1)
            entropy = dist.entropy().sum(axis=-1)
            return action, log_prob, entropy, self.critic(x)
        else:
            raise NotImplementedError(f"Espaço de ações do tipo {type(self.action_space_type)} não suportado.")
...
class PPOTrainer:
    def __init__(self, agent, envs, args):
        ...
        # Determina se o espaço de ação é contínuo ou discreto
        ...
        for iteration in range(1, self.args.num_iterations + 1):
            ...
            for epoch in range(self.args.update_epochs):
                ...
                for start in range(0, self.args.batch_size, self.args.minibatch_size):
                    ...
                    # Para ações contínuas, a logprob é somada ao longo das dimensões
